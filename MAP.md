...
28 packages, 52,344 lines of wac, 1053 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 2,898 | 59 | `bytes` |
...
| [`tor`](packages/tor/) | A Tor client, in wac, on top of this repo's TLS 1.3 stack. | 4,837 | 121 | `bytes` `codec` `crypto` `datetime` `platform` `std` `tls` |
