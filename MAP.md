...
32 packages, 64,945 lines of wac, 1330 tests,
...
| [`crypto`](packages/crypto/) | SHA-256, SHA-512/384, SHA-3, SHAKE, keccak256, HMAC, HKDF, bcrypt_pbkdf, ChaCha20-Poly1305, AES-CTR, AES-GCM, X25519, Ed25519, NIST P-256 and… | 5,614 | 149 | `bignum` |
...
| [`tor`](packages/tor/) | Tor in wac — a client, a relay, a directory authority and an onion-service client, on top of this repo's TLS 1.3 stack. | 13,802 | 255 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
