...
28 packages, 60,670 lines of wac, 1171 tests,
34 command-line programs and 4 browser pages.
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 2,980 | 80 | `bytes` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 6,611 | 31 | `bytes` `fmt` `fs` `platform` `std` |
...
| [`tor`](packages/tor/) | A Tor client, in wac, on top of this repo's TLS 1.3 stack. | 11,465 | 193 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
