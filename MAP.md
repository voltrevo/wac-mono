...
28 packages, 59,096 lines of wac, 1132 tests,
32 command-line programs and 4 browser pages.
...
| [`http`](packages/http/) | HTTP/1.1: parsing requests and responses, and writing both. | 914 | 28 | `bytes` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 6,509 | 22 | `bytes` `fmt` `platform` `std` |
...
| [`tor`](packages/tor/) | A Tor client, in wac, on top of this repo's TLS 1.3 stack. | 10,038 | 180 | `bytes` `codec` `crypto` `datetime` `fmt` `http` `platform` `std` `tls` `wactest` |
