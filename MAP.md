...
24 packages, 40,917 lines of wac, 896 tests,
...
| [`box`](packages/box/) | Fifty-nine applets in one program, chosen by the first argument. | 3,051 | 20 | `bytes` `codec` `crypto` `datetime` `gzip` `http` `json` `platform` `regex` `server` `sh` `tls` `unicode` `url` `zstd` |
