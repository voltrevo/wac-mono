...
26 packages, 49,296 lines of wac, 991 tests,
...
| [`platform`](packages/platform/) | A capability world for wac applications, so a program can be written **entirely in wac** — no TypeScript of its own — and still read files, tell… | 2,459 | 58 | `bytes` |
...
| [`sh`](packages/sh/) | A shell, in wac. | 5,482 | 18 | `bytes` `platform` `std` |
...
| [`ssz`](packages/ssz/) | **Merkleization and Merkle-branch verification work. | 567 | 12 | `crypto` |
| [`tls`](packages/tls/) | TLS 1.3 (RFC 8446) in wac. | 3,263 | 70 | `crypto` |
| [`box`](packages/box/) | 60 applets in one program, chosen by the first argument. | 4,253 | 32 | `bytes` `codec` `crypto` `datetime` `gzip` `http` `json` `platform` `regex` `server` `sh` `std` `tls` `unicode` `url` `zstd` |
